...
class TrafficSignal:
    ...
    def switch_phase(self, next_idx: int) -> None:
        if next_idx == self.current_phase_index:
            return
    ...
    def _compute_duration(self, queue_lengths: Dict[EdgeKey, float]) -> float:
    ...
    def set_phase(self, phase_index: int, duration: float) -> None:
        if phase_index != self.current_phase_index:
            self.switch_phase(phase_index)
        self.target_duration = duration


def build_signals(network: RoadNetwork) -> Dict[int, TrafficSignal]:
    ...
    for node_id, node in network.nodes.items():
        ...
        signal = TrafficSignal(node_id=node_id, phases=phases)
